...
def select_additional_inputs():
    global select_additional_inputs
    select_additional_inputs = filedialog.askopenfilename(title="Select reference file")
    if select_additional_inputs:
        select_additional_inputs.config(text="Selected Reference File: " + get_file_name_with_extension(select_additional_inputs))
    else:
        reference_file_label.config(text="No reference file selected.")
    ...
    if selected_script_files and selected_reference_file:
        ...
        if output_file:
            ...
            if selected_additional_inputs:
                additional_inputs.extend(selected_additional_inputs.split())
